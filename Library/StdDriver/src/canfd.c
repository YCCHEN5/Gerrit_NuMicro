...
void CANFD_EnableInt(CANFD_T *psCanfd, uint32_t u32IntLine0, uint32_t u32IntLine1, uint32_t u32TXBTIE, uint32_t u32TXBCIE)
{
    ...
    if(u32IntLine0 != 0)
    {
        ...
        psCanfd->IE = psCanfd->IE | u32IntLine0;
        /* Enable CAN FD specified interrupt */
        psCanfd->ILE = psCanfd->ILE | CANFD_ILE_ENT0_Msk;
    }
    if(u32IntLine1 != 0)
    {
        /*Setting the CANFD0_IRQ1 Interrupt*/
        psCanfd->ILS = psCanfd->ILS | u32IntLine1;
        /* Enable CAN FD specified interrupt */
        psCanfd->ILE = psCanfd->ILE | CANFD_ILE_ENT1_Msk;
    }

    /*Setting the Tx Buffer Transmission Interrupt Enable*/
    psCanfd->TXBTIE = psCanfd->TXBTIE | u32TXBTIE;

    /*Tx Buffer Cancellation Finished Interrupt Enable*/
    psCanfd->TXBCIE = psCanfd->TXBCIE | u32TXBCIE;
}
...
uint32_t CANFD_TransmitDMsg(CANFD_T *psCanfd, uint32_t u32TxBufIdx, CANFD_FD_MSG_T *psTxMsg)
{
    ...
    if(psTxMsg->bBitRateSwitch)
        psTxBuffer->u32Config |= TX_BUFFER_T1_ELEM_BRS_Msk;
    ...
    if(psTxMsg->bTimestampCaptured)
    {
        psTxBuffer->u32Config |= TX_BUFFER_T1_ELEM_TSCE_Msk;
    }
...
/**
 * @brief       Read TX Event FIFO Massage
 *
 * @param[in]   psCanfd               The pointer to CAN FD module base address.
...
uint32_t CANFD_ReadTxFifoEventMsg(CANFD_T *psCanfd, uint8_t u8TxEvntIdx, CANFD_TX_EVNT_ELEM_T *psEventFifoMsgBuf)
{
    ...
    {
        if (((psCanfd->TXEFS >> CANFD_TXEFS_EFFL_Pos) & 0x3F) > 0)
        {
            uint8_t GetIndex;

            GetIndex = (uint8_t)((psCanfd->TXEFS >> CANFD_TXEFS_EFGI_Pos) & 0x1F);
...
void CANFD_ClearStatusFlag(CANFD_T *psCanfd, uint32_t u32InterruptFlag)
{
    ...
    psCanfd->IR = psCanfd->IR | u32InterruptFlag;
}
 ...
 *
 * @details     This function is used to switch between the normal and initial modes.
 */
int32_t CANFD_RunToNormal(CANFD_T *psCanfd, uint8_t u8Enable)
{
    ...
    {
        /* start operation */
        psCanfd->CCCR = psCanfd->CCCR & ~(CANFD_CCCR_CCE_Msk | CANFD_CCCR_INIT_Msk);
    ...
    {
        /* init mode */
        psCanfd->CCCR = psCanfd->CCCR | CANFD_CCCR_INIT_Msk;
